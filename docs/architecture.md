...
- **Exactly-once completion.** `OnRequestCompleted` fires exactly once per top-level request — streaming or non-streaming, success, failure, cancellation, or abandoned enumeration; late callers get the same report instance, and no request-path event is delivered after it. Late MCP progress notifications are dropped on a best-effort basis — one racing completion may rarely be delivered after it. The report attached to a non-streaming response is the *same instance* the observer received.
