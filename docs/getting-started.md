...
A typical rendering of the events for one tool call — the middleware emits nothing until it observes something, so the stream opens with the `ToolInvoking` header:
...
| `HeaderFormatter` | `Func<ToolDescriptor, string>?` | `null` | Formats tool headers. Default: "Calling {DisplayName} Tool" for functions, "Calling {Source} MCP" for [MCP tools](mcp.md), "Calling {DisplayName} Agent" for [agent tools](agents.md) — the kind word is dropped when the name already ends with it (case-insensitive), so "Andes Test MCP" or "Research Agent" is not doubled. |
